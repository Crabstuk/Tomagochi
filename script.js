let hunger =
	localStorage.getItem("hunger") == undefined
		? 5
...
let tamagotchiName = localStorage.getItem("tamagotchiName")
	? localStorage.getItem("tamagotchiName")
	: null
...
let lastGotFood =
	localStorage.getItem("lastGotFood") || new Date().toISOString()
let lastGotWater =
	localStorage.getItem("lastGotWater") || new Date().toISOString()
...
// Toggle screen on/off animation
...
// Show or hide Pong minigame
const showMinigame = () => {
	pong.style.display = pong.style.display === "block" ? "none" : "block"
}
...
PlayButton.addEventListener("click", showMinigame)
...
const CheckWhatEmotion = () => {
	if (hunger == 0 || thirst == 0) return Emotions.dead
	if (hunger >= 3 && thirst < 3) return Emotions.thirsty
	if (hunger < 3 && thirst >= 3) return Emotions.hungry
	if (hunger >= 7 && thirst >= 7) return Emotions.happy
	if (hunger >= 3 && thirst >= 3) return Emotions.inDifferent
	if (hunger < 3 && thirst < 3) return Emotions.angry
}
...
function update() {
	...
	if (playerScore >= 3 || AIScore == 3) {
		playerScore = 0
		AIScore = 0
		let gameIsRunning = canvas.style.display !== "none"
		if (gameIsRunning) addFun()
